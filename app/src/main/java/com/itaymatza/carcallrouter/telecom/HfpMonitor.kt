...
class HfpMonitor(private val context: Context, private val changed: () -> Unit) : AutoCloseable {
    ...
    fun start() {
        if (!Access.bluetoothGranted(context)) { changed(); return }
        try {
        ...
        } catch (e: RuntimeException) { RouterLog.event("HFP_ERROR", e.javaClass.simpleName) }
    }
    fun refresh() {
        if (closed) return
        try {
