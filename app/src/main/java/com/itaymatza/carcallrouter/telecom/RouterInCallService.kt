...
import com.itaymatza.carcallrouter.Access
import com.itaymatza.carcallrouter.ProjectionMonitor
import com.itaymatza.carcallrouter.RouterLog
import com.itaymatza.carcallrouter.RouterSettings
import com.itaymatza.carcallrouter.SessionBridge
...
class RouterInCallService : InCallService(), SessionBridge.Control {
    ...
    private fun startObservers() {
        ...
        hfp = HfpMonitor(this) {
            ...
            if (guardHasActed() && (address == null || !hfp.known || address !in hfp.connected)) {
                suspendSessionFromEvent("BMW HFP disappeared; this session stays paused")
            }
            queueEvaluation()
        }
    ...
    override fun onCallAudioStateChanged(audioState: CallAudioState) {
        ...
        if (guardHasActed() && router.supported(audioState, settings.targetAddress) == null) {
            suspendSessionFromEvent("BMW disappeared from Telecom devices; this session stays paused")
        }
        ...
        return when (audio?.route) {
            CallAudioState.ROUTE_BLUETOOTH -> {
                ...
                when {
                    ...
                    address.equals(settings.competitorAddress, true) -> RoutingPolicy.Route.COMPETING_CAR
