...
object PolicyCases {
    private fun policy(initial: Route = Route.COMPETING_CAR, manual: Boolean = false) =
    ...
    fun cases(): List<Pair<String, () -> Unit>> = listOf(
        ...
        "no BMW device means no request" to { check(!policy().evaluate(Snapshot(0,targetAvailable=false)).requestTarget) },
        ...
        "BMW callback verifies" to { val p=policy(); p.evaluate(Snapshot(0)); p.evaluate(Snapshot(100,route=Route.TARGET)); check(p.verified) },
        ...
        "known head unit reassertion bounded" to { val p=policy(); p.evaluate(Snapshot(0)); p.evaluate(Snapshot(100,route=Route.TARGET)); check(p.evaluate(Snapshot(350)).requestTarget); p.evaluate(Snapshot(500,route=Route.TARGET)); check(p.evaluate(Snapshot(700)).requestTarget); p.evaluate(Snapshot(800,route=Route.TARGET)); check(!p.evaluate(Snapshot(1100)).requestTarget); check(p.requests==3) },
        ...
        "Buds override respected" to { val p=policy(); p.evaluate(Snapshot(0,route=Route.TARGET)); p.evaluate(Snapshot(200,route=Route.OTHER_BLUETOOTH)); check(p.phase==Phase.SUSPENDED) },
        ...
        "unknown change after BMW is not fought" to { val p=policy(); p.evaluate(Snapshot(0,route=Route.TARGET)); p.evaluate(Snapshot(100,route=Route.UNKNOWN)); check(p.phase==Phase.SUSPENDED) },
        ...
        "BMW disconnect cancels guard" to { val p=policy(); p.evaluate(Snapshot(0)); p.evaluate(Snapshot(200,targetAvailable=false)); check(p.phase==Phase.SUSPENDED) },
        ...
        "manual test still requires BMW" to { check(!policy(manual=true).evaluate(Snapshot(0,targetAvailable=false)).requestTarget) },
        ...
        "new session resets pause and counters" to { val p=policy(); p.evaluate(Snapshot(0)); p.suspend("pause"); p.begin(9000,Route.COMPETING_CAR); check(p.evaluate(Snapshot(9000)).requestTarget); check(p.requests==1) }
    ...
    private fun auditCases(): List<Pair<String, () -> Unit>> = listOf(
        ...
        "speaker event cannot be erased by later queued car event" to {
            val p = policy()
            p.evaluate(Snapshot(0))
            p.observeRoute(Route.SPEAKER)
            p.observeRoute(Route.COMPETING_CAR)
            check(!p.evaluate(Snapshot(1600)).requestTarget)
            check(p.phase == Phase.SUSPENDED)
        },
        "Buds event cannot be erased while waiting for projection" to {
            val p = policy()
        ...
        "override while BMW availability is pending: $route" to {
            val p = policy()
