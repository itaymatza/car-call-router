...
object PolicyCases {
    ...
    fun cases(): List<Pair<String, () -> Unit>> = listOf(
        ...
        "target-device disconnect cancels guard" to { val p=policy(); p.evaluate(Snapshot(0)); p.evaluate(Snapshot(200,targetAvailable=false)); check(p.phase==Phase.SUSPENDED) },
        ...
        "manual test still requires target device" to { check(!policy(manual=true).evaluate(Snapshot(0,targetAvailable=false)).requestTarget) },
