...
object PolicyCases {
    ...
    fun cases(): List<Pair<String, () -> Unit>> = listOf(
        ...
        "device arriving after deadline does not trigger" to { val p=policy(); p.evaluate(Snapshot(0,targetAvailable=false)); check(!p.evaluate(Snapshot(10001,targetAvailable=true)).requestTarget) },
        ...
        "known competitor retry after timeout" to { val p=policy(); p.evaluate(Snapshot(0)); check(p.evaluate(Snapshot(2500)).requestTarget); check(p.requests==2) },
        "explicit timeout outcome retries only after request gap" to { val p=policy(); val first=p.evaluate(Snapshot(0)); p.requestFailed(requireNotNull(first.requestAttempt),RoutingPolicy.RequestError.TIMEOUT); check(!p.evaluate(Snapshot(100)).requestTarget); check(p.evaluate(Snapshot(2500)).requestTarget) },
        "endpoint gone waits for a fresh endpoint revision" to { val p=policy(); val first=p.evaluate(Snapshot(0,endpointRevision=4)); p.requestFailed(requireNotNull(first.requestAttempt),RoutingPolicy.RequestError.ENDPOINT_GONE); check(!p.evaluate(Snapshot(2500,endpointRevision=4)).requestTarget); check(p.evaluate(Snapshot(2600,endpointRevision=5)).requestTarget) },
        "another request cancellation stops automation" to { val p=policy(); val first=p.evaluate(Snapshot(0)); p.requestFailed(requireNotNull(first.requestAttempt),RoutingPolicy.RequestError.CANCELLED_BY_OTHER); check(p.phase==Phase.SUSPENDED); check(!p.evaluate(Snapshot(3000)).requestTarget) },
        "late callback from replaced attempt is ignored" to { val p=policy(); val first=p.evaluate(Snapshot(0)); val second=p.evaluate(Snapshot(2500)); p.requestFailed(requireNotNull(first.requestAttempt),RoutingPolicy.RequestError.CANCELLED_BY_OTHER); check(p.phase==Phase.VERIFYING); p.evaluate(Snapshot(2600,route=Route.TARGET)); check(p.verified); check(second.requestAttempt==2) },
        "successful outcome still requires endpoint observation" to { val p=policy(); val first=p.evaluate(Snapshot(0)); p.requestSucceeded(requireNotNull(first.requestAttempt),100); check(!p.evaluate(Snapshot(599)).requestTarget); p.evaluate(Snapshot(600)); check(p.phase==Phase.FAILED) },
        "successful outcome plus target observation verifies" to { val p=policy(); val first=p.evaluate(Snapshot(0)); p.requestSucceeded(requireNotNull(first.requestAttempt),100); p.evaluate(Snapshot(200,route=Route.TARGET)); check(p.verified) },
        "late evidence receives a full action window" to { val p=policy(); p.evaluate(Snapshot(0,targetAvailable=false)); check(p.evaluate(Snapshot(9000,targetAvailable=true)).requestTarget); check(!p.evaluate(Snapshot(10001)).requestTarget); check(p.phase==Phase.VERIFYING) },
        "unknown competitor never gets blind retry" to { val p=policy(Route.OTHER_BLUETOOTH); p.evaluate(Snapshot(0,route=Route.OTHER_BLUETOOTH)); check(!p.evaluate(Snapshot(2500,route=Route.OTHER_BLUETOOTH)).requestTarget) },
        "known competing-device reassertion bounded" to { val p=policy(); p.evaluate(Snapshot(0)); p.evaluate(Snapshot(100,route=Route.TARGET)); check(!p.evaluate(Snapshot(350)).requestTarget); check(p.evaluate(Snapshot(2500)).requestTarget); p.evaluate(Snapshot(2600,route=Route.TARGET)); check(p.evaluate(Snapshot(5000)).requestTarget); p.evaluate(Snapshot(5100,route=Route.TARGET)); check(!p.evaluate(Snapshot(5500)).requestTarget); check(p.requests==3) },
        "early known competitor bounce waits for request gap" to { val p=policy(); p.evaluate(Snapshot(0)); p.evaluate(Snapshot(50,route=Route.TARGET)); val d=p.evaluate(Snapshot(100)); check(!d.requestTarget); check(d.wakeAt==2500L) },
        "speaker override is debounced then respected" to { val p=policy(); p.evaluate(Snapshot(0)); p.evaluate(Snapshot(100,route=Route.TARGET)); check(!p.evaluate(Snapshot(200,route=Route.SPEAKER)).requestTarget); check(p.phase!=Phase.SUSPENDED); p.evaluate(Snapshot(1000,route=Route.SPEAKER)); check(p.phase==Phase.SUSPENDED) },
        ...
        "new alternative during pending request is debounced then respected" to { val p=policy(); p.evaluate(Snapshot(0)); check(!p.evaluate(Snapshot(100,route=Route.SPEAKER)).requestTarget); check(p.phase!=Phase.SUSPENDED); p.evaluate(Snapshot(1000,route=Route.SPEAKER)); check(p.phase==Phase.SUSPENDED) },
        ...
        "temporary projection uncertainty does not cancel guard" to { val p=policy(); p.evaluate(Snapshot(0)); p.evaluate(Snapshot(100,route=Route.TARGET)); check(!p.evaluate(Snapshot(200,projection=null,route=Route.COMPETING_DEVICE)).requestTarget); check(p.phase!=Phase.SUSPENDED); check(p.evaluate(Snapshot(2500,projection=true,route=Route.COMPETING_DEVICE)).requestTarget) },
        "temporary HFP uncertainty does not cancel guard" to { val p=policy(); p.evaluate(Snapshot(0)); p.evaluate(Snapshot(100,route=Route.TARGET)); check(!p.evaluate(Snapshot(200,targetHfpConnected=null,route=Route.COMPETING_DEVICE)).requestTarget); check(p.phase!=Phase.SUSPENDED); check(p.evaluate(Snapshot(2500,targetHfpConnected=true,route=Route.COMPETING_DEVICE)).requestTarget) },
        "transient endpoint-list gap does not cancel guard" to { val p=policy(); p.evaluate(Snapshot(0)); p.evaluate(Snapshot(100,route=Route.TARGET)); check(!p.evaluate(Snapshot(200,targetAvailable=false,route=Route.COMPETING_DEVICE)).requestTarget); check(p.phase!=Phase.SUSPENDED); check(p.evaluate(Snapshot(2500,targetAvailable=true,route=Route.COMPETING_DEVICE)).requestTarget) },
        ...
        "startup window releases control" to { val p=policy(); p.evaluate(Snapshot(0)); p.evaluate(Snapshot(100,route=Route.TARGET)); p.evaluate(Snapshot(5500,route=Route.TARGET)); check(p.phase==Phase.RELEASED); check(!p.evaluate(Snapshot(5600)).requestTarget) },
        ...
        "request exception prevents retries" to { val p=policy(); val d=p.evaluate(Snapshot(0)); p.requestFailed(requireNotNull(d.requestAttempt), RoutingPolicy.RequestError.RUNTIME_EXCEPTION); check(!p.evaluate(Snapshot(2600)).requestTarget) },
