...
fun main(args:Array<String>){
 ...
 "temporary_projection_unknown_recovers_without_cancelling_guard" to {Fixture().use{f->f.established();TestQueue.now=350;ProjectionMonitor.emit(null);f.flush();f.route(competing);f.flush();ProjectionMonitor.emit(true);f.flush();countEquals(f,2)}},
 "temporary_hfp_unknown_recovers_without_cancelling_guard" to {Fixture().use{f->f.established();TestQueue.now=350;HfpMonitor.emit(emptySet(),known=false);f.flush();f.route(competing);f.flush();HfpMonitor.emit(setOf(TARGET,COMPETING));f.flush();countEquals(f,2)}},
 "transient_endpoint_gap_recovers_without_cancelling_guard" to {Fixture().use{f->f.established();TestQueue.now=350;f.endpoints(listOf(competing));f.flush();f.route(competing);f.flush();f.endpoints(listOf(target,competing,other));f.flush();countEquals(f,2)}},
 ...
 "coalesced_telecom_target_gap_recovers_guard" to {Fixture().use{f->f.established();TestQueue.now=350;f.endpoints(listOf(competing));f.endpoints(listOf(target,competing,other));f.route(competing);f.flush();countEquals(f,2)}},
 ...
 "api37_own_request_after_endpoint_change_is_not_an_override" to {Fixture().use{f->f.active();f.route(target);f.flush();f.service.onCallEndpointRequested(target);f.flush();check(!SessionBridge.status.contains("SUSPENDED"));TestQueue.now=350;f.route(competing);f.flush();countEquals(f,2)}},
 ...
 "api37_multiple_own_requests_are_consumed_individually" to {Fixture().use{f->f.established();TestQueue.now=350;f.route(competing);f.flush();countEquals(f,2);f.service.onCallEndpointRequested(target);f.service.onCallEndpointRequested(target);f.flush();check(!SessionBridge.status.contains("SUSPENDED"))}},
