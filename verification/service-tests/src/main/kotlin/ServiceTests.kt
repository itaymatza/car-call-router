...
private fun reset(){
 ...
 HfpMonitor.instances.clear();HfpMonitor.isKnown=true;HfpMonitor.devices=setOf(TARGET,COMPETING,OTHER);HfpMonitor.audioDevices=emptySet()
...
fun main(args:Array<String>){
 ...
 "temporary_projection_unknown_recovers_without_cancelling_guard" to {Fixture().use{f->f.established();TestQueue.now=350;ProjectionMonitor.emit(null);f.flush();f.route(competing);f.flush();ProjectionMonitor.emit(true);f.flush();TestQueue.advanceTo(2500);countEquals(f,2)}},
 "temporary_hfp_unknown_recovers_without_cancelling_guard" to {Fixture().use{f->f.established();TestQueue.now=350;HfpMonitor.emit(emptySet(),known=false);f.flush();f.route(competing);f.flush();HfpMonitor.emit(setOf(TARGET,COMPETING));f.flush();TestQueue.advanceTo(2500);countEquals(f,2)}},
 "transient_endpoint_gap_recovers_without_cancelling_guard" to {Fixture().use{f->f.established();TestQueue.now=350;f.endpoints(listOf(competing));f.flush();f.route(competing);f.flush();f.endpoints(listOf(target,competing,other));f.flush();TestQueue.advanceTo(2500);countEquals(f,2)}},
 "positive_known_competitor_retries_are_bounded" to {Fixture().use{f->f.established();for(t in listOf(2500L,5000L)){TestQueue.advanceTo(t);f.route(competing);f.flush();f.route(target);f.flush()};countEquals(f,3)}},
 "deadline_stops_further_requests" to {Fixture().use{f->f.established();TestQueue.advanceTo(5500);f.route(competing);f.flush();countEquals(f,1)}},
 ...
 "coalesced_telecom_target_gap_recovers_guard" to {Fixture().use{f->f.established();TestQueue.now=350;f.endpoints(listOf(competing));f.endpoints(listOf(target,competing,other));f.route(competing);f.flush();TestQueue.advanceTo(2500);countEquals(f,2)}},
 ...
 "api37_callback_has_exact_platform_signature" to {val m=RouterInCallService::class.java.getDeclaredMethod("onCallEndpointRequested",CallEndpoint::class.java);check(m.returnType==Void.TYPE)},
 "api37_own_request_before_endpoint_change_is_not_an_override" to {Fixture().use{f->f.active();f.service.onCallEndpointRequested(target);f.flush();check(!SessionBridge.status.contains("SUSPENDED"));f.route(target);f.flush();check(SessionBridge.status.contains("STABILIZING"))}},
 "api37_own_request_after_endpoint_change_is_not_an_override" to {Fixture().use{f->f.active();f.route(target);f.flush();f.service.onCallEndpointRequested(target);f.flush();check(!SessionBridge.status.contains("SUSPENDED"));TestQueue.advanceTo(2500);f.route(competing);f.flush();countEquals(f,2)}},
 "api37_external_endpoint_request_suspends_the_session" to {Fixture().use{f->f.active();f.service.onCallEndpointRequested(speaker);f.flush();check(SessionBridge.status.contains("SUSPENDED"));TestQueue.advanceTo(2000);countEquals(f,1)}},
 "api37_multiple_own_requests_are_consumed_individually" to {Fixture().use{f->f.established();TestQueue.advanceTo(2500);f.route(competing);f.flush();countEquals(f,2);f.service.onCallEndpointRequested(target);f.service.onCallEndpointRequested(target);f.flush();check(!SessionBridge.status.contains("SUSPENDED"))}},
 "structured_trace_requires_endpoint_and_exact_hfp_audio_evidence" to {Fixture().use{f->f.active();f.route(target);f.flush();var traces=RouterLog.events.filter{it.first=="ROUTING_TRACE"}.map{it.second};check(traces.any{it.contains("event=TELECOM_ENDPOINT_CONFIRMED")});check(traces.none{it.contains("event=TARGET_HFP_AUDIO_CONFIRMED")});HfpMonitor.audioDevices=setOf(TARGET);HfpMonitor.emit(HfpMonitor.devices);f.flush();traces=RouterLog.events.filter{it.first=="ROUTING_TRACE"}.map{it.second};check(traces.any{it.contains("event=TARGET_HFP_AUDIO_CONFIRMED")})}},
 "authorization_revocation_during_request_stays_suspended_after_restore" to {Fixture().use{f->f.active();countEquals(f,1);Access.authorization=false;TestQueue.advanceTo(1500);check(SessionBridge.status.contains("AUTHORIZATION_MISSING"));Access.authorization=true;f.route(competing);f.flush();TestQueue.advanceTo(4000);countEquals(f,1)}},
 "runtime_permission_revocation_during_request_stays_suspended_after_restore" to {Fixture().use{f->f.active();countEquals(f,1);Access.runtime=false;TestQueue.advanceTo(1500);check(SessionBridge.status.contains("UNSAFE_CALL"));Access.runtime=true;f.route(competing);f.flush();TestQueue.advanceTo(4000);countEquals(f,1)}},
 "process_recreation_preserves_settings_but_does_not_retake_active_call" to {Fixture().use{f->f.established();val saved=RouterSettings(f.service);check(saved.enabled);check(saved.targetAddress==TARGET);f.service.onDestroy();val recovered=RouterInCallService();recovered.currentCallEndpoint=competing;recovered.onCreate();recovered.onBind(Intent());recovered.onAvailableCallEndpointsChanged(mutableListOf(target,competing,other));recovered.onCallEndpointChanged(competing);recovered.onCallAdded(f.call);TestQueue.runReady();val restored=RouterSettings(recovered);check(restored.enabled);check(restored.targetAddress==TARGET);check(recovered.issuedRequests.isEmpty());check(SessionBridge.status.contains("ALREADY_ACTIVE_BIND"));recovered.onDestroy()}}
 ,"aosp_watchdog_never_overlaps_requests" to {Fixture().use{f->f.service.autoCompleteRequests=false;f.active();countEquals(f,1);TestQueue.advanceTo(2499);countEquals(f,1);TestQueue.advanceTo(2500);countEquals(f,2);check(!SessionBridge.status.contains("SUSPENDED"))}}
 ,"timeout_error_retries_after_gap" to {Fixture().use{f->f.service.autoCompleteRequests=false;f.active();f.service.failLatest(CallEndpointException.ERROR_REQUEST_TIME_OUT);f.flush();TestQueue.advanceTo(2499);countEquals(f,1);TestQueue.advanceTo(2500);countEquals(f,2)}}
 ,"endpoint_gone_requires_fresh_snapshot" to {Fixture().use{f->f.service.autoCompleteRequests=false;f.active();f.service.failLatest(CallEndpointException.ERROR_ENDPOINT_DOES_NOT_EXIST);f.flush();TestQueue.advanceTo(2500);countEquals(f,1);f.endpoints(listOf(target,competing,other));f.flush();countEquals(f,2)}}
 ,"another_request_error_suspends" to {Fixture().use{f->f.service.autoCompleteRequests=false;f.active();f.service.failLatest(CallEndpointException.ERROR_ANOTHER_REQUEST);f.flush();check(SessionBridge.status.contains("SUSPENDED"));TestQueue.advanceTo(5000);countEquals(f,1)}}
 ,"accepted_outcome_without_endpoint_does_not_retry" to {Fixture().use{f->f.active();TestQueue.advanceTo(500);countEquals(f,1);check(SessionBridge.status.contains("FAILED"))}}
 ,"completed_call_persists_last_session_result" to {Fixture().use{f->f.established();HfpMonitor.audioDevices=setOf(TARGET);HfpMonitor.emit(HfpMonitor.devices);f.flush();f.service.onCallRemoved(f.call);f.flush();val last=checkNotNull(RouterSettings(f.service).lastSession);check(last.phase=="STABILIZING");check(last.reason=="TARGET_ENDPOINT_STABILIZING");check(last.confirmation=="TARGET_HFP_AUDIO")}}
 )
