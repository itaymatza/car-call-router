...
if (-not $SkipBuild) {
    ...
    & (Join-Path $Root 'gradlew.bat') "-PAPP_APPLICATION_ID=$ApplicationId" "-PAPP_VERSION_CODE=$VersionCode" "-PAPP_VERSION_NAME=$VersionName" ':core:check' ':app:assembleDebug' ':app:testDebugUnitTest' ':verification:service-tests:run' ':app:lintDebug' '--console=plain'
