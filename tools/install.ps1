...
if (-not $VersionCode) { $VersionCode = '11' }
if (-not $VersionName) { $VersionName = '0.3.0-beta.9' }
...
if (-not $SkipBuild) {
    ...
    & (Join-Path $Root 'gradlew.bat') "-PAPP_APPLICATION_ID=$ApplicationId" "-PAPP_VERSION_CODE=$VersionCode" "-PAPP_VERSION_NAME=$VersionName" ':core:check' ':app:ktlintCheck' ':app:assembleDebug' ':app:testDebugUnitTest' ':verification:service-tests:ktlintCheck' ':verification:service-tests:run' ':app:lintDebug' '--console=plain'
